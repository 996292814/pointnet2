    ...
    for image_file in image_files:
        ...
        if os.path.exists(annotation_file):
            ...
            for idx, annotation in enumerate(annotations):
                ...
                output_path = os.path.join(output_folder+"\images", f"split{idx}_{image_file}")
                ...
                # 构建保存路径，并保存只包含标注实例的图像，同时保留原图颜色和特征
                output_path = os.path.join(output_folder + "\depth",  f'split{idx}_{image_file.split("_rgb")[0]}_pdepth.png')
                cv2.imwrite(output_path, blank_depth_image)

                # 获取 npy 文件中的坐标数据
                npy_file_path = os.path.join(folder_path, f'{image_file.split("_rgb")[0]}_label.npy')

                # 提取图像特征
                features = extract_features(blank_image, (x1, y1), (x2, y2), segmentation, blank_depth_image)
                ...
                if os.path.exists(npy_file_path):
                    ...
                    for coord in npy_data:
                        ...
                        if is_point_inside_bbox((x, y), bbox):
                            print('saved:', f'split{idx}_{image_file.split("_rgb")[0]}.pcd')
                            # 使用主成分分析点云的朝向
                            pca = PCA.PCA(pointcloud_folder+f'\\split{idx}_{image_file.split("_rgb")[0]}.pcd')
                            # 构建 CSV 行数据
                            csv_data = [f'split{idx}_{image_file.split("_rgb")[0]}.pcd',
                                        coord[1]]
                            ...
                            # 构建 CSV 行数据
                            features_data = [f'split{idx}_{image_file.split("_rgb")[0]}.pcd',
                                        features['bbox_width'],
...
def process_images_in_folders(root_folder, output_folder, annotation_root_folder,pointcloud_folder):
    # 获取根文件夹下的所有子文件夹
    ...
    for subfolder in subfolders:
        ...
        # 处理子文件夹中的图像
        process_images_in_folder(folder_path, output_folder, annotation_root_folder,pointcloud_folder)

# 指定待处理数据的文件夹路径
root_folder = r"D:\3DPointCloud\ICRA2022\dataset-1\3\3"
output_folder = r"D:\3DPointCloud\ProcessedData\features"
annotation_root_folder = r"D:\3DPointCloud\ICRA2022\annotations\dyson_annotations\dyson_annotations\3"
pointcloud_folder = r"D:\3DPointCloud\ProcessedData\filter\pcd"
# 处理所有子文件夹中的图像
process_images_in_folders(root_folder, output_folder, annotation_root_folder, pointcloud_folder)

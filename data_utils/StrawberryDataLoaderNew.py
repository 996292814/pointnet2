...
class OwnPointCloudDataLoader(Dataset):
    def __init__(self, root, args, split='train', process_data=False):
        ...
        self.classes = {cat: int(cat) for idx, cat in enumerate(self.cat)}
        shape_ids = {}
        if self.num_category == 10:
        ...
        print('shapenames:')
        # print(shape_names)
        self.datapath = [(shape_names[i], os.path.join(self.root, shape_names[i].split('_')[0], shape_ids[split][i]) + '.txt') for i
        ...
        if self.uniform:
            ...
            self.save_path = os.path.join(root,
                                          'ownpointcloud%d_%s_%dpts.dat' % (self.num_category, split, self.npoints))
    ...
    def _get_item(self, index):
        if self.process_data:
            point_set, weight = self.list_of_points[index], self.list_of_labels[index]
        else:
            fn = self.datapath[index]
            # print(fn)
            # print(self.datapath[index][0].split('_')[0])
            cls = self.classes[self.datapath[index][0].split('_')[0]]
            # print(cls)
            weight = np.array([cls]).astype(np.int32)
            point_set = np.loadtxt(fn[1], delimiter=' ').astype(np.float32)
        ...
        # 处理图片
        image_path = os.path.join( r"D:\3DPointCloud\ProcessedData\new\images", '_'.join(fn[0].split('_')[1:-1])+'_rgb.png')
        preprocessed_image = preprocess_image(image_path)
        return point_set, preprocessed_image, weight[0]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--num_category', default=37, type=int, choices=[4, 10, 40], help='training on ModelNet10/40')
    ...
    data = OwnPointCloudDataLoader(r"D:\3DPointCloud\ProcessedData\ModelNet-filter-augmented",args, split='train')

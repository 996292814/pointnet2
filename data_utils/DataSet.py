...
def process_images_in_folder(folder_path, output_folder, annotation_root_folder):
                    ...
                    for coord in npy_data:
                        if len(coord) == 7:
                            ...
                            if is_point_inside_bbox((x, y), bbox):
                                # 构建 CSV 行数据
                                csv_data = [f'4_split{idx}_{image_file.split("_rgb")[0]}.pcd', coord[1]]
...
def process_images_in_folders(root_folder, output_folder, annotation_root_folder):
    ...
    for subfolder in subfolders:
        ...
        # 处理子文件夹中的图像
        process_images_in_folder(folder_path, output_folder, annotation_root_folder)

# 指定待处理数据的文件夹路径
root_folder = r"D:\3DPointCloud\ICRA2022\dataset-1\4\4"
output_folder = r"D:\3DPointCloud\ProcessedData\4"

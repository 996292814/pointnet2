...
def parse_args():
    ...
    parser.add_argument('--log_dir', type=str, default='2024-04-03_17-22', required=False, help='Experiment root')
...
def evaluate_model(model, data_loader, tolerances):
    ...
    with torch.no_grad():
        for points, images, target in tqdm(data_loader, total=len(data_loader)):
            if not args.use_cpu:
                points, images, target = points.cuda(), images.cuda(), target.cuda()
            # 将输入数据的维度转置，以符合模型输入的格式要求。
            points = points.transpose(2, 1)
            pred, _ = model(points, images)
            # 计算预测值与真实值之间的相对误差
